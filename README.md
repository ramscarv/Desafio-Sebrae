...
Abra um novo terminal e navegue até o frontend:
cd frontend
...
Retorna status HTTP 409 (Conflict) com mensagem clar
...
Backend não conecta (CORS)
Problema: Erro de CORS no console do navegador
Solução: Verifique se a classe WebConfig está presente no backend:
...
Problema: Web server failed to start. Port 8080 was already in use.
...
proxy: {
  '/api': {
    target: 'http://localhost:8081',  // Porta alterada
    changeOrigin: true
  }
}
...
npm install
npm run dev
Erro ao criar pedido (estoque)
Problema: "Estoque insuficiente para o produto"

Solução:
Verifique a quantidade disponível do produto
Ajuste a quantidade do pedido
Ou aumente o estoque do produto

Banco de dados H2 não abre
Problema: Não consigo acessar o console H2

Solução: Verifique as configurações:
properties
